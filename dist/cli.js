...
function lineCommentToMarkdown(line) {
    const match = line.match(/^\s*\/\/\s?(.*)$/);
    return match ? match[1] : null;
}
function blockCommentStart(line) {
    return line.match(/^(\s*)\/\*(\*)?/);
}
function stripBlockCommentDecoration(line) {
    return line.replace(/^\s+\* ?/, "");
}
function countLeadingWhitespace(line) {
    return line.match(/^\s*/)?.[0].length ?? 0;
}
function stripBlockIndent(line, indent) {
    return indent && line.startsWith(indent) ? line.slice(indent.length) : line;
}
function normalizeBlockCommentLines(lines, indent) {
    ...
    return undecoratedLines.map((line) => line.trim() === "" ? "" : line.slice(commonIndent));
}
function getMarkdownCommentLines(source) {
    const markdownLines = [];
    let blockCommentLines = null;
    let blockCommentIndent = "";
    function flushBlockComment() {
        if (blockCommentLines) {
            markdownLines.push(...normalizeBlockCommentLines(blockCommentLines, blockCommentIndent));
    ...
    for (const line of source.split(/\r?\n/)) {
        ...
        markdownLines.push(...normalizeBlockCommentLines([afterOpen.slice(0, endIndex)], start[1] ?? ""));
    }
    flushBlockComment();
...
function sourceToMarkdown(source, language) {
    const lines = source.split(/\r?\n/);
    const parts = [styleBlock];
    let codeBuffer = [];
    let markdownBuffer = [];
    let blockCommentLines = null;
    let blockCommentIndent = "";
    ...
    function flushBlockComment() {
        ...
        for (const markdown of normalizeBlockCommentLines(blockCommentLines, blockCommentIndent)) {
...
console.log(`Generated ${generatedCount} literated Markdown file(s). Skipped ${skippedCount} unmarked source file(s).`);
